...
#pragma GCC dignostic error "-Wtype-limits"
namespace tmuduo
{
...
void LogStream::formatInteger(T v)
{
	if (buffer_.avail() >= kMaxNumericSize)
	{
		...
		buffer_.add(len); // 将cur_向后移动len长度
	}
}

LogStream& LogStream::operator<<(int v)
{
	formatInteger(v); // 隐藏的this指针
	return *this;
}

LogStream& LogStream::operator<<(short v)
{
	*this << static_cast<int>(v);  // 将v转为int，然后调用LogStream& LogStream::oeprator<< (int v)
	return *this;
}
...
LogStream& LogStream::operator<<(const void* p) // 指针类型运算
{
