...
{
  loop_->assertInLoopThread(); // 目前无法跨线程使用
...
std::vector<TimerQueue::Entry> TimerQueue::getExpired(Timestamp now)
{
  ...
  Entry sentry(now, reinterpret_cast<Timer*>(UINTPTR_MAX)); // 这个哨兵的地址是最大的
  // 返回第一个未到期的Timer的迭代器
  // lower_bound的含义是返回第一个值>=sentry的元素的iterator
  // reinterpret_cast<Timer*>(UINTPTR_MAX)最大的地址值
...
void TimerQueue::reset(const std::vector<Entry>& expired, Timestamp now)
{
  ...
  if (nextExpire.valid()) // 成功插入后，应该判断是否是更早的到来的定时器事件
  {
    // 重置定时器的超时时刻(timerfd_settime)
